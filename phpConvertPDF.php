...
{	
    ...
    // Title and Invoice date
		if (isset($_SESSION['language']) && $_SESSION['language'] == "en") {
			if (isset($_SESSION['company_vat_payer']) && $_SESSION['company_vat_payer'] == 1) {
			$ino = 'VAT Invoice No. ' . $_SESSION['prefix'] . sprintf('%05d', $_SESSION['invoiceNo']);
			} else {
			$ino = 'Invoice No. ' . $_SESSION['prefix'] . sprintf('%05d', $_SESSION['invoiceNo']);
			}
			$idate = 'Issue date: ' . $_SESSION['date'];
		} else {
			if (isset($_SESSION['company_vat_payer']) && $_SESSION['company_vat_payer'] == 1) {
			$ino = 'PVM sąskaita faktūra Serija ' . $_SESSION['prefix'] . ' Nr. ' . sprintf('%05d', $_SESSION['invoiceNo']);
			} else {
			$ino = 'Sąskaita faktūra Serija ' . $_SESSION['prefix'] . ' Nr. ' . sprintf('%05d', $_SESSION['invoiceNo']);
			}
...
function Footer()
{
    ...
    $this->SetFont('DejaVu','',10);
	$this->Cell(0,5,$swift . $_SESSION['bankSWIFT'],0,2,'L');
...
function InvoiceLines()
{
	...
	if (isset($_SESSION['company_vat_payer']) && $_SESSION['company_vat_payer'] == 1) {
		...
		$cellWidth = array(15, 50, 25, 25, 30, 45);
	}
    ...
    for ($i = 0; $i <= $_SESSION['count']; $i++)  {
		...
		if (isset($_SESSION['company_vat_payer']) && $_SESSION['company_vat_payer'] == 1) {
			$this->Cell($cellWidth[5],5,number_format(floatval($_SESSION['vatPercentage']), 2, ',', ''),'LR',0,'R',$fill);
