...
if (isset($_GET['sid'])) {
	...
	$countries = $conn->query("SELECT *FROM classifier_country WHERE CountryCode='" . $company["Country"] . "'");
...
if ($customer["Flat"] != "") { $_SESSION["customer_address1"] .= "-" . $customer["Flat"]; }
	...
	$countryresult = $conn->query("SELECT *FROM classifier_country WHERE CountryCode='" . $customer["Country"] . "'");
